...
# New Base Code V10
#
# Original code Source - this is a very helpfull link, try it
...
Hardware:
Raspbery PI ver 3B+ or ver 4
3 x MAX6675 SPI thermocouple interface PCB with thermocoupl probes
3 x Heater band outputs (GPIO pins). these would be connected to SSR-40 DA solid state relays 3volts
1 x pizo buzzer
optional 7" touch screen

It incorporates 3 x PID contronlers, with a graphical animated live display of temperature and PID power
each PID has a run button, target temperature, PID variables, output on/off display, and alarm output
it can be put into Manual control, allowing the heater outputs to be turned on/off individualy
It also writes configuration data to disk file. later development will allow for the saved data to be used to
load profiles for different plastic types
'''
...
# write event log to disk file, pid values, alarms etc
...
#---------------------------------------------------------------------------------------
# target1 value changed in GUI, Entry changed call back
...
#------------------------------------------------------------------------------------
# P1 value changed in GUI, Entry changed call back
...
#-----------------------------------------------------------------------------------
# I1 value changed in GUI, Entry changed call back
...
#-----------------------------------------------------------------------------------
# D1 value changed in GUI, Entry changed call back
...
#----------------------------------------------------------------------------------------
# target2 value changed in GUI, Entry changed call back
...
#---------------------------------------------------------------------------------
# D2 value changed in GUI, Entry changed call back
...
#-------------------------------------------------------------------------------------
# target3 value changed in GUI, Entry changed call back
...
#------------------------------------------------------------------------------
# D3 value changed in GUI, Entry changed call back
...
# Dummy function prevents segfault, clicking 'quit' comes here
def _destroy(event):
    HEATER1.off()
    HEATER2.off()
    HEATER3.off()
