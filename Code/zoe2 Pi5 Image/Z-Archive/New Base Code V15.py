...
helptxt = """
1-9-20  V15       By Richard Nicholson, New Zealand
...
open_defaults_file = True

vertual_keyboard = False
...
def savefileas():
    ...
    if save_text_as:                    #Do if True
        ...
        filename = save_text_as.name                    #set filename to save_text_asso we can use 'Save' button
...
#-----------------------------------------------------------------------------------------
# Button: Config  window, Display config window usedto load base config values
def config():
    ...
    label_con9 = tk.Label(configwin, text='Default Data Folder location : ', font=dfont, bg='white')
    ...
    label_con9.grid(row=10, column=1, sticky=tk.E)                      # default folder location
...
def PID1_run():
    ...
    if (manual_state == False):                             # If manual OFF...then change PID run state
        if (PID1_alarm_state == True):                   # If f PID alarm....
...
# load file Defaults.txt, on startup uses open_defaults_file = True to trigger, on not to
openfile()
