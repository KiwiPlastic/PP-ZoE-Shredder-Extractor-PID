...
helptxt = """
1-9-20  V14       By Richard Nicholson, New Zealand
...
It incorporates 3 x PID controlers, with a graphical live display of temperature and PID power output.
...
# floats for read file and GUI display update on file read
...
# file handeling tags
filename = ""
open_folder = "/home/pi/Documents/Python/Data"
...
def openfile():
    ...
    try:
        if filename:
            ...
            data = the_file.readline()                      # read one line of data. Date & time saved last (notused)
            print ('1 = ', (data))
            ...
            the_file.close()                                        #close file and update GUI ......

            entry_filename.delete(0, 8)
...
def savefileas():
    ...
    if save_text_as:                    #Do if True
        ...
        filename = save_text_as.name                    #set filename so we can do use 'Save'
...
def config():
    ...
    #configwin.geometry('950x900')          #let it self size, its better result
    ...
    label_con9 = tk.Label(configwin, text='Default File Folder: ', font=dfont, bg='white')
    ...
    label_con1.grid(row=2, column=1, sticky=tk.E)                       # grahp update int
    ...
    label_con9.grid(row=10, column=1, sticky=tk.E)                      # default folder
    label_con10.grid(row=11, column=3, sticky=tk.E)                   # blank
    label_conB2.grid(row=13, column=3, sticky=tk.E)                   # blank
    ...
    entry_open_folder.grid(row=10, column=2, sticky=tk.E)

    button_save_close.grid(row = 12, column = 3)
...
entry_filename =tk.Entry(frame, font=dfont, bg='white',  width=40)
entry_filename.insert(0, filename)
...
button_load_file.grid(row = 0 , column  = 6)
button_save_file.grid(row = 0, column = 7)
button_save_file_as.grid(row = 0, column = 8 )
